...
void loop_uart(void) {
	if(uart3_get_ok) {
		if(uart3_mode == 1) {					    //命令模式
			parse_cmd(uart_receive_buf);			
		} else if(uart3_mode == 2) {			//单个舵机调试
			parse_action(uart_receive_buf);
		} else if(uart3_mode == 3) {		  //多路舵机调试
			parse_action(uart_receive_buf);
		} else if(uart3_mode == 4) {		  //存储模式
...
void car_set1(int car_left1,int car_right1,int car_left2,int car_right2) {
	//总线马达设置	
	sprintf((char *)cmd_return, "{#006P%04dT2000!#007P%04dT2000!#008P%04dT2000!#009P%04dT2000!}", 
		(int)(1500+car_left1), (int)(1500-car_right1),(int)(1500+car_left2), (int)(1500-car_right2));
	zx_uart_send_str(cmd_return);
	return;
}
...
void parse_cmd(u8 *cmd) {
  ...
  uint16_t float1;
	...
	{
		if(sscanf((char *)uart_receive_buf, "$SMODE%d-%d!", &int1, &float1)) 
		{
			mode = int1;
			diff = float1;
			clamp = 1;
			sprintf((char *)uart_receive_buf, "mode = %d\n", int1);
			uart1_send_str(uart_receive_buf);
			
//			sprintf((char *)uart_receive_buf, "diff = %d\r\n", diff);
//			uart1_send_str(uart_receive_buf);
			//beep_on_times(1,100);
		}
